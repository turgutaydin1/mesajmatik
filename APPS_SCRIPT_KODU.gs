...
{a:"#073f35",b:"#0d6a54",c:"#e2c66f",d:"#fffdf7",e:"#8fc7ad",stil:0},
{a:"#102f3d",b:"#195c63",c:"#d5b55f",d:"#fbfaf4",e:"#8fb7bd",stil:1},
{a:"#2f4236",b:"#65705b",c:"#e0c27b",d:"#fffdf8",e:"#b9c4a7",stil:2},
{a:"#3b273a",b:"#70485c",c:"#dab86a",d:"#fff9f3",e:"#c59bae",stil:3},
{a:"#173c2d",b:"#3d6b45",c:"#d8bc6c",d:"#fffdf5",e:"#9fc398",stil:4},
{a:"#183747",b:"#315a70",c:"#d7bc77",d:"#fffdf8",e:"#9db9c6",stil:5},
{a:"#443721",b:"#796642",c:"#e3c77c",d:"#fffaf0",e:"#c9b98d",stil:6},
{a:"#243d38",b:"#2b7066",c:"#d9bd6a",d:"#fffdf8",e:"#8dc7bd",stil:7}
];return t[no%t.length]}
function kartZeminCiz(ctx,t,no){var g=ctx.createLinearGradient(0,0,1080,1080);g.addColorStop(0,t.a);g.addColorStop(1,t.b);ctx.fillStyle=g;ctx.fillRect(0,0,1080,1080);var rg=ctx.createRadialGradient(540,320,20,540,320,520);rg.addColorStop(0,"rgba(255,255,255,.08)");rg.addColorStop(1,"rgba(255,255,255,0)");ctx.fillStyle=rg;ctx.fillRect(0,0,1080,1080);ctx.strokeStyle=t.c;ctx.lineWidth=5;ctx.strokeRect(52,52,976,976);ctx.strokeStyle="rgba(255,255,255,.16)";ctx.lineWidth=1.5;ctx.strokeRect(70,70,940,940);ctx.save();ctx.globalAlpha=.18;ctx.strokeStyle=t.e;ctx.lineWidth=2;if(no%4===0){for(var i=0;i<5;i++){ctx.beginPath();ctx.arc(110+i*215,105,18+i*2,0,Math.PI*2);ctx.stroke();ctx.beginPath();ctx.arc(970-i*215,975,18+i*2,0,Math.PI*2);ctx.stroke()}}else if(no%4===1){for(var y=120;y<=960;y+=120){ctx.beginPath();ctx.moveTo(78,y);ctx.lineTo(98,y-10);ctx.lineTo(118,y);ctx.stroke();ctx.beginPath();ctx.moveTo(962,y);ctx.lineTo(982,y-10);ctx.lineTo(1002,y);ctx.stroke()}}else if(no%4===2){for(var q=0;q<7;q++){var x=105+q*145;ctx.beginPath();ctx.moveTo(x,86);ctx.lineTo(x+10,96);ctx.lineTo(x,106);ctx.lineTo(x-10,96);ctx.closePath();ctx.stroke();ctx.beginPath();ctx.moveTo(x,974);ctx.lineTo(x+10,984);ctx.lineTo(x,994);ctx.lineTo(x-10,984);ctx.closePath();ctx.stroke()}}else{for(var s=0;s<6;s++){ctx.beginPath();ctx.arc(95,175+s*140,12,0,Math.PI*2);ctx.stroke();ctx.beginPath();ctx.arc(985,175+s*140,12,0,Math.PI*2);ctx.stroke()}}ctx.restore()}
...
function kartCiz(){var c=el("cardCanvas"),ctx=c.getContext("2d"),no=__tasarimNo%8,t=kartTemasi(no),gun=el("gunSecim").value,parca=kartMetniniAyir();ctx.clearRect(0,0,1080,1080);kartZeminCiz(ctx,t,no);ctx.fillStyle="rgba(0,0,0,.10)";ctx.beginPath();ctx.arc(540,145,64,0,Math.PI*2);ctx.fill();ctx.strokeStyle=t.c;ctx.lineWidth=2;ctx.beginPath();ctx.arc(540,145,55,0,Math.PI*2);ctx.stroke();ctx.fillStyle=t.c;ctx.textAlign="center";ctx.font="58px Georgia";ctx.fillText(gunSimgesi(gun),540,164);ctx.fillStyle=t.d;ctx.font="bold 46px Arial";ctx.fillText(gun,540,244);ctx.strokeStyle="rgba(255,255,255,.20)";ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(250,270);ctx.lineTo(830,270);ctx.stroke();var maxH=parca.imza?540:610,fit=metniKartaSigdir(ctx,parca.govde,820,maxH),startY=335;ctx.font=fit.size+"px Georgia";ctx.textAlign="left";ctx.fillStyle=t.d;for(var i=0;i<fit.lines.length;i++){var line=fit.lines[i];if(line===""){startY+=Math.round(fit.lineHeight*.65)}else{var isLast=(i===fit.lines.length-1||fit.lines[i+1]==="");ikiYanaYaz(ctx,line,130,startY,820,isLast);startY+=fit.lineHeight}}if(parca.imza){ctx.textAlign="right";ctx.fillStyle=t.c;ctx.font="italic "+Math.max(24,Math.min(32,fit.size))+"px Georgia";ctx.fillText(parca.imza,950,910)}ctx.textAlign="center";ctx.font="20px Arial";ctx.fillStyle=t.c;ctx.fillText("Mesajmatik",540,984)} function tasarimDegistir(){__tasarimNo=(__tasarimNo+1)%8;kartCiz()} function gorselKaydet(){kartCiz();var a=document.createElement("a");a.href=el("cardCanvas").toDataURL("image/png");a.download="mesajmatik-kart.png";a.click()} function gorselPaylas(){kartCiz();el("cardCanvas").toBlob(function(blob){if(!blob)return;var f=new File([blob],"mesajmatik-kart.png",{type:"image/png"});if(navigator.canShare&&navigator.canShare({files:[f]}))navigator.share({files:[f],title:"Mesajmatik"}).catch(function(){});else gorselKaydet()},"image/png")}
</script></body></html>`;
}
