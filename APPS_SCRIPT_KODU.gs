...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .actions{display:grid;grid-template-columns:repeat(3,1fr);gap:9px;margin-top:11px}.actions button{border:0;border-radius:10px;padding:11px 8px;font-weight:700;cursor:pointer;color:#fff;background:#2d6ea3}
  .actions button:nth-child(2){background:#7da493}.actions button:nth-child(3){background:#c49325}.actions button:nth-child(4){background:#23895e}.actions button:nth-child(5){background:#3267b5}.actions button:nth-child(6){background:#222}.actions button:nth-child(7){background:#2196c9}.actions button:nth-child(8){background:#6d8f80}
  ...
  @media(max-width:650px){.grid{grid-template-columns:1fr}.full{grid-column:auto}.actions{grid-template-columns:1fr 1fr}.cardActions{grid-template-columns:1fr}h1{font-size:29px}}
</style>
</head>
<body>
<div class="wrap">
...
<main>
<div class="grid">
<div><label for="gunSecim">Özel Gün / Zaman</label><select id="gunSecim"><option>Berat Kandili</option><option>Kadir Gecesi</option><option>Mevlid Kandili</option><option>Regaib Kandili</option><option>Miraç Kandili</option><option>Ramazan Bayramı</option><option>Kurban Bayramı</option><option>Cuma Günü</option><option>Ramazan Ayı</option><option>Arefe Günü</option></select></div>
...
<div class="actions"><button onclick="kopyala()">📋 Kopyala</button><button onclick="paylas()">📤 Paylaş</button><button onclick="gorselKartOlustur()">🖼️ Görsel Kart</button><button onclick="whatsapp()">WhatsApp</button><button onclick="facebook()">Facebook</button><button onclick="xPaylas()">X</button><button onclick="telegram()">Telegram</button><button onclick="instagram()">Instagram / Diğer</button></div></div>
<div id="cardBox" class="section cardBox"><h2>Paylaşım Kartı</h2><canvas id="cardCanvas" width="1080" height="1080"></canvas><div class="cardActions"><button onclick="gorselKaydet()">⬇️ Görseli Kaydet</button><button onclick="gorselPaylas()">📤 Görseli Paylaş</button><button onclick="tasarimDegistir()">🎨 Tasarımı Değiştir</button></div><div class="note">Görsel kart 1080×1080 hazırlanır.</div></div>
</main></div>
<script>
...
function wrapText(ctx,text,x,y,maxWidth,lineHeight,maxLines){var words=text.split(/\s+/),line="",lines=[];for(var i=0;i<words.length;i++){var test=line?line+" "+words[i]:words[i];if(ctx.measureText(test).width>maxWidth&&line){lines.push(line);line=words[i];if(lines.length>=maxLines-1)break}else line=test}if(line&&lines.length<maxLines)lines.push(line);for(var j=0;j<lines.length;j++)ctx.fillText(lines[j],x,y+j*lineHeight)}
...
function kartCiz(){var c=el("cardCanvas"),ctx=c.getContext("2d"),palettes=[["#0f6048","#e6c979","#fffdf7"],["#143f50","#c7a351","#f7fbfc"],["#315744","#d6b568","#fffaf0"]],p=palettes[__tasarimNo%palettes.length],g=ctx.createLinearGradient(0,0,1080,1080);g.addColorStop(0,p[0]);g.addColorStop(1,"#0b2f26");ctx.fillStyle=g;ctx.fillRect(0,0,1080,1080);ctx.strokeStyle=p[1];ctx.lineWidth=8;ctx.strokeRect(48,48,984,984);ctx.fillStyle=p[1];ctx.font="70px Georgia";ctx.textAlign="center";ctx.fillText("☾",540,155);ctx.fillStyle=p[2];ctx.font="bold 50px Arial";ctx.fillText(el("gunSecim").value,540,235);ctx.font="34px Georgia";ctx.textAlign="left";wrapText(ctx,metin(),125,365,830,54,10);ctx.textAlign="center";ctx.font="24px Arial";ctx.fillStyle=p[1];ctx.fillText("Mesajmatik",540,970)} function tasarimDegistir(){__tasarimNo++;kartCiz()} function gorselKaydet(){kartCiz();var a=document.createElement("a");a.href=el("cardCanvas").toDataURL("image/png");a.download="mesajmatik-kart.png";a.click()} function gorselPaylas(){kartCiz();el("cardCanvas").toBlob(function(blob){if(!blob)return;var f=new File([blob],"mesajmatik-kart.png",{type:"image/png"});if(navigator.canShare&&navigator.canShare({files:[f]}))navigator.share({files:[f],title:"Mesajmatik"}).catch(function(){});else gorselKaydet()},"image/png")}
</script></body></html>`;
}
